...
const config = require('../config/local'),
    ...
    strategyConfig = config.strategies.horseinplaylayfield2;
...
module.exports = {
    ...
    tradeMarket: function(session, currentMarket, marketBook) {
        ...
        return promise.coroutine(function*() {
            ...
            let layPrice = Number(Number(favouritePrice / 2.00).toFixed(2));
            let validPrice = new betfair.BetfairPrice(layPrice);
            layPrice = validPrice.price

            logger.log(`Favourite price ${favouritePrice}`, 'info');
            logger.log(`Calculated lay price ${layPrice}`, 'info');

            const accountFunds = yield account.getAccountFunds(session);

            if (favouritePrice > 2.00) {
                ...
                if (accountFunds.result.availableToBetBalance > 2) {
                    ...
                    if (strategyConfig.excludeRunners.indexOf(activeRunners) === -1) {
                        ...
                        if (strategyConfig.placeOrders) {
                            logger.log(`${currentMarket.description} - Laying the field of ${activeRunners} runners at ${layPrice} for £${stake}`, 'info');
                            _this.placeLayOrders(session, currentMarket, marketBook.runners, layPrice, stake);
                        } else {
    ...
    processTodaysRaces: function(session) {
        ...
        return promise.coroutine(function*() {
            ...
            // Filter races to exclude by venue and class and include by distance
            const tradeRaces = _.filter(races.result, function(race) {
                let raceClass = race.marketName.substr(race.marketName.indexOf(' ') + 1);
                let raceDistance = race.marketName.split(' ')[0]
                //return ((strategyConfig.excludeVenues.indexOf(race.event.venue) === -1) && (strategyConfig.excludeClasses.indexOf(raceClass) === -1));
                return ((strategyConfig.excludeVenues.indexOf(race.event.venue) === -1) && (strategyConfig.excludeClasses.indexOf(raceClass) === -1) && strategyConfig.includeDistances.indexOf(raceDistance) >= 0);
            });

            for (let race of tradeRaces) {
                logger.log(`Trading - ${race.description}`, 'info');
            }
            ...
            if (currentMarket && (currentMarket.raceStatus !== meeting.raceStatus)) {
                ...
                switch (currentMarket.raceStatus) {
                    case 'ATTHEPOST':
                        ...
                        break;
                    case 'GOINGBEHIND':
                        this.goingBehind(session, currentMarket);
                        break;
                    case 'OFF':
    ...
    goingBehind: function(session, currentMarket) {
        const _this = this;
        logger.log(`${currentMarket.description} - Going Behind`, 'info');
